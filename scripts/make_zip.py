...
def  main():
  ...
  installer = "\installer\SerialController Installer.exe"
   
  if demo:
    installer = "\installer\SerialController Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\SerialController manual.pdf" 
  ]
  ...
  ZIPNAME = "SerialController-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "SerialController-v" + FULLVERSIONSTR + "-win-demo.zip"
